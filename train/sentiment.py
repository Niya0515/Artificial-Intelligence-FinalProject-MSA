...
def single_train(model, optimizer, lr_scheduler, num_epochs):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        cnt = 0
        # Iterate over data.
        for inputs1, labels in tr_loader:
        ...
        test_loss, test_acc, test_f1, _, _ = single_evaluate(model, vl_loader)
...
def single_evaluate(model, loader):
    model.eval()
    test_loss = 0
    all_preds = []
    all_labels = []
...
def predict_label(model, loader):
    model.eval()
    all_preds = []
...
## Load splits
train_df = pd.read_csv(dloc+'splits/train.txt', header=None)
val_df = pd.read_csv(dloc+'splits/val.txt', header=None)
test_df = pd.read_csv(dloc+'splits/test.txt', header=None)

tr_ids = train_df[0].to_numpy().flatten()
vl_ids = val_df[0].to_numpy().flatten()
te_ids = test_df[0].to_numpy().flatten()

lab_train = train_df[1].to_numpy().flatten()
lab_val = val_df[1].to_numpy().flatten()
lab_test = test_df[1].to_numpy().flatten()

# Read features
if ttype != 'none':
    ## Read text features
    feats_text = json.load(open('features/%s_ht%d.json'%(ttype, htag),'r'))
...
if vtype != 'none':
    ...
    feats_img, vdim = get_visual_feats(vtype, ftype, htag)
...
if ttype != 'none' and vtype != 'none':
    ...
    tr_data = SingleDataset2(ft_tr_img, lab_train, normalize)
    vl_data = SingleDataset2(ft_vl_img, lab_val, normalize)
    te_data = SingleDataset2(ft_te_img, lab_test, normalize)
else:
    tr_data = SingleDataset2(ft_tr_txt, lab_train, normalize)
    vl_data = SingleDataset2(ft_tr_txt, lab_val, normalize)
    te_data = SingleDataset2(ft_tr_txt, lab_test, normalize)
...
if ttype == 'none':
    model_ft = SingleMLP_2Mod(vdim)
elif vtype == 'none':
    model_ft = SingleMLP_2Mod(tdim)
else:
...
if ttype == 'none' or vtype == 'none':
    model_ft, best_epoch = single_train(model_ft, optimizer_ft, scheduler, num_epochs=epochs)
    torch.save(model_ft.state_dict(), 'saved_models/singlemlp2_%s_%s_%s_%s_ht%d.pt'%(vtype, ftype, ttype, layer, htag))
    te_loss, te_acc, te_f1, all_preds, all_labels = single_evaluate(model_ft, te_loader)
else:
    model_ft, best_epoch = train(model_ft, optimizer_ft, scheduler, num_epochs=epochs)
    torch.save(model_ft.state_dict(), 'saved_models/multimlp2_%s_%s_%s_%s_ht%d.pt'%(vtype, ftype, ttype, layer, htag))
...
# preds = predict_label(model_ft, te_loader)
# f = open('preds.txt', 'w')
# f.write(str(preds))
# f.close()
