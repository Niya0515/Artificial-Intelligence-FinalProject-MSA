...
def get_visual_feats(vtype, ftype, htag):
    if vtype == 'places':
        feats_img = json.load(open('features/places.json', 'r'))[ftype]
        vdim = 2048 if ftype == 'feats' else 365
    elif vtype == 'emotion':
        feats_img = json.load(open('features/emotion.json', 'r'))[ftype]
        vdim = 2048 if ftype == 'feats' else 8
    elif vtype == 'imagenet':
        feats_img  = json.load(open('features/imagenet.json', 'r'))[ftype]
        vdim = 2048 if ftype == 'feats' else 1000
    elif vtype == 'clip':
        feats_img  = json.load(open('features/clip_ht%d.json'%(htag), 'r'))['img_feats']
        vdim = 512
    else:
        feats_img = json.load(open('features/faces.json','r'))[ftype]
...
class SingleMLP_2Mod(nn.Module):
    def __init__(self, dim):
        super(SingleMLP_2Mod, self).__init__()
        
        self.fc1 = nn.Linear(dim, 128)
        self.bn1 = nn.BatchNorm1d(128)
        self.cf = nn.Linear(128, 3)

        self.dp1 = nn.Dropout(0.5)
        self.relu = nn.ReLU()

    def forward(self, x1):
        x1 = self.dp1(self.relu(self.bn1(self.fc1(x1))))
