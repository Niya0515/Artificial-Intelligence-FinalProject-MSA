...
def get_resnet_feats():
    ...
    if args.vtype == 'imagenet':
        ...
        print('places')
        model_file = 'pretrained_models/resnet101_places_best.pth.tar'
        ...
        print('emotion')
        model_file = 'pretrained_models/best_emo_resnet50.pt'
    ...
    dataset = MMDataset(img_transforms, txt_transform, txt_processor)
...
def get_clip_feats():
    ...
    dataset = MMDataset(img_transform=img_preprocess, txt_transform=txt_transform, txt_processor=txt_processor)
    dt_loader = DataLoader(dataset, batch_size=128, sampler=SequentialSampler(dataset))
...
if args.vtype != 'clip':
    ...
    json.dump({'feats': feats, 'logits': logits}, open('features/%s.json'%(args.vtype), 'w'))
else:
    img_feats, text_feats = get_clip_feats()
    print(np.array(img_feats).shape, np.array(text_feats).shape)
    json.dump({'img_feats': img_feats, 'text_feats': text_feats}, open('features/%s_ht%d.json'%(args.vtype,args.ht), 'w'))