...
class MMDataset(Dataset):
    def __init__(self, img_transform=None, txt_transform=None, txt_processor=None):
        self.file_names = pd.read_csv(os.path.join(dloc,'filenames.txt'), header=None)
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(os.path.join(dloc, 'image_with_text', fname+'.jpg')).convert('RGB')
        text = open(os.path.join(dloc, 'image_with_text', fname+'.txt'), 'r', encoding='utf-8', errors='ignore').read().strip().lower()
...
def process_text(text, text_processor):
    ...
    proc_text = text_processor.pre_process_doc(text)

    clean_text = [word.strip() for word in proc_text if not re.search(r"[^a-z0-9.,\s]+", word)]

    clean_text = [word for word in clean_text if word not in ['rt', 'http', 'https', 'htt']]
