...
ph_data = MMDataset(txt_transform=txt_transform, txt_processor=txt_processor)
ph_loader = DataLoader(ph_data, batch_size=1, sampler=SequentialSampler(ph_data))

## Loader batch size is 1 because getting features according to text's length.
## Not padding all texts to the same length as done usually during training a sequential model
...
json.dump(embed_dict, open('features/%s_ht%d.json'%(args.btype, args.ht), 'w'))
